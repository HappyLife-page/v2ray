...
vless_ws_path="/`pwgen -csn 9 8 | xargs |sed 's/ /\//g'`"
trojan_ws_path="/`pwgen -csn 9 8 | xargs |sed 's/ /\//g'`"
socks_ws_path="/`pwgen -csn 9 8 | xargs |sed 's/ /\//g'`"
shadowsocks_ws_path="/`pwgen -csn 9 8 | xargs |sed 's/ /\//g'`"

# 使用gRPC配置vless,trojan,socks,shadowsocks协议
# 4.分别随机生成vless,trojan,socks,shadowsocks需要使用的grpc的path
vless_grpc_path="$(pwgen -scny -r "\!@#$%^&*()-+={}[]|:\";',/?><\`~" 56 1)"
trojan_grpc_path="$(pwgen -scny -r "\!@#$%^&*()-+={}[]|:\";',/?><\`~" 56 1)"
socks_grpc_path="$(pwgen -scny -r "\!@#$%^&*()-+={}[]|:\";',/?><\`~" 56 1)"
shadowsocks_grpc_path="$(pwgen -1scn 16)$(pwgen -1scny -r "\!@#$%^&*()-+={}[]|:\";',/?><\`~" 36)"
