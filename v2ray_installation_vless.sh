...
uuid="`uuidgen`"
path="/`pwgen -A0 8 5 | xargs |sed 's/ /\//g'`"
...
##配置系统时区为东八区(其实如果只做代理用的话没什么必要,反而会导致你本地时间和代理时间不一致)
rm -f /etc/localtime
...
##安装acme,并申请加密证书
## ssl_dir="`mkdir -pv /usr/local/etc/v2ray/ssl | awk -F"'" 'END{print $2}'`"
...
chmod www-data.www-data $ssl_dir/v2ray.*


##创建WS路径,配置v2ray时会用到[目录可以自定义]
